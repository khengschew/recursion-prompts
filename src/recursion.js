...
var capitalizeFirst = function(array) {
  if (array.length === 0) {
    ...
    var newStr = array[0][0].toUpperCase() + array[0].slice(1, array[0].length);
    return [newStr].concat(capitalizeFirst(array.slice(1, array.length)));
  }
};
...
var nestedEvenSum = function(obj) {
  if ((typeof obj === 'number') && (obj % 2 === 0)) {
    ...
    return 0 + nestedEvenSum(obj[keys[0]]) + nestedEvenSum(rObj);
  } else {
    return 0;
  }
};
  ...
  if (!Array.isArray(array)) {
    ...
    if (array.length > 0) {
        for (var i = 0; i < array.length; i++) {
          retArr = retArr.concat(flatten(array[i]));
        }
    }
...
var compress = function(list) {
  if (list.length === 0) {
    ...
    return [].concat(compress(list.slice(1, list.length)));
  } else {
...
var minimizeZeroes = function(array) {
  if (array.length === 0) {
    ...
    return [].concat(minimizeZeroes(array.slice(1, array.length)));
  } else {
